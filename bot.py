...
async def show_files(message, where="", order="date DESC"):
    if not is_auth(message.from_user.id):
        ...
        return
    async with aiosqlite.connect(DB_NAME) as db:
        q = f"SELECT file_id, file_name, category, size, date FROM files WHERE 1=1 {where} ORDER BY {order}"
    ...
    for file_id, name, cat, size, date in rows:
        ...
        caption = f"{'🎬' if cat=='video' else '🖼️' if cat=='photo' else '🤖' if cat=='apk' else '🍎' if cat=='ipa' else '📄'} {name}\n{mb} MB | {date}"
        try:
            if cat == "video":
...
async def cmd_search(message: Message):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT file_id, file_name, category, size, date FROM files WHERE file_name LIKE ?",
            (keyword,)
        ) as cursor:
            rows = await cursor.fetchall()
